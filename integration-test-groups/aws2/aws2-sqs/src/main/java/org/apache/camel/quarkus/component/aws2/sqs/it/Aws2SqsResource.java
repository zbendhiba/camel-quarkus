...
public class Aws2SqsResource {
    ...
    @Path("/sqs/queue/autocreate/{queueName}")
    @POST
    @Produces(MediaType.APPLICATION_JSON)
    public List<String> autoCreateDelayedQueue(@PathParam("queueName") String queueName)
            throws Exception {
        // queue creation without any operation resulted in 405 status code
        return producerTemplate.requestBody(
                "aws2-sqs://" + queueName
                        + "?autoCreateQueue=true&operation=listQueues",
                null,
    ...
    @Path("/sqs/queues/{queueName}")
    @GET
    @Produces(MediaType.APPLICATION_JSON)
    public List<String> listQueues(@PathParam("queueName") String queueName) throws Exception {
        return producerTemplate.requestBody(componentUri(queueName) + "?operation=listQueues", null, ListQueuesResponse.class)
                .queueUrls();
    }
